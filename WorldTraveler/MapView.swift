...
struct MapView: View {
    ...
    static var regions: [MKCoordinateRegion] = [
        MKCoordinateRegion(
            center: CLLocationCoordinate2D(latitude: 41.0, longitude: 36.0),
            span: MKCoordinateSpan(latitudeDelta: 4.0, longitudeDelta: 4.0)
            ),
        MKCoordinateRegion(
            center: CLLocationCoordinate2D(latitude: 51.0, longitude: -0.11),
            span: MKCoordinateSpan(latitudeDelta: 4.0, longitudeDelta: 4.0)
            )
        ]
            ...
            .overlay(
                VStack
                {
                    Picker("Picker", selection: $selectedIndex, content: {
                            Text("MAP_CITY_SAMSUN").tag(0)
                            Text("MAP_CITY_LONDRA").tag(1)
                        }
